...
import net.opentsdb.search.schemas.tsmeta.AnalyzedAndMappedTSMetaSchema;
...
public class TestAnalyzedAndMappedTSMetaSchema {
  ...
  @Test
  public void ctor() {
    final AnalyzedAndMappedTSMetaSchema schema = 
        new AnalyzedAndMappedTSMetaSchema(es);
    assertEquals(doc_type, schema.docType());
  }
  
  @Test
  public void index() throws Exception {
    final AnalyzedAndMappedTSMetaSchema schema = new AnalyzedAndMappedTSMetaSchema(es);
    ...
    assertTrue(payload.contains("\"kv_map\":[\"TAGK_01=TAGV_01\"]"));
    assertTrue(payload.contains("\"kv_map_name\":[\"TAGK_host=TAGV_web01\"]"));
    assertTrue(payload.contains("\"keys\":["));
    ...
    assertEquals(HOST + "/" + doc_type + "/" + "_doc"
